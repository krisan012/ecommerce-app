            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ $product->name }}" class="form-input mt-1 block w-full dark:bg-gray-700 dark:text-gray-300" required>
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-gray-700 dark:text-gray-300">Description:</label>
                        <textarea name="description" class="form-input mt-1 block w-full dark:bg-gray-700 dark:text-gray-300" required>{{ $product->description }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label for="price" class="block text-gray-700 dark:text-gray-300">Price:</label>
                        <input type="number" step="0.01" name="price" value="{{ $product->price }}" class="form-input mt-1 block w-full dark:bg-gray-700 dark:text-gray-300" required>
                    </div>
                    <div class="mb-4">
                        <label for="stock" class="block text-gray-700 dark:text-gray-300">Stock:</label>
                        <input type="number" name="stock" value="{{ $product->stock }}" class="form-input mt-1 block w-full dark:bg-gray-700 dark:text-gray-300" required>
                    </div>
                    <div class="mb-4">
